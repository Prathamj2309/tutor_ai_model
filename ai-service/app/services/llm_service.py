"""
LLM Service - wraps llama-cpp-python to run a local GGUF model.
The model is loaded once at startup (singleton) to avoid reload overhead.
"""
import os
import json
import re
    ...
    if _llm is None:
        ...
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                ...
                "Place your GGUF file in ai-service/models/ and update .env"
            )
...
def build_messages(subject: str, history: list, question: str) -> list:
...
def extract_topic_tags(llm, question: str, answer: str) -> list:
...
def generate_answer(question: str, history: list, subject: str = "general") -> dict:
    """
    Generate an AI answer for the given question and history.
...
def generate_quiz(weak_topics: list, subject: str) -> dict:
    """
    Generate a 5-question multiple choice quiz targeting the given weak topics.
    Returns: { questions: [...] }
    """
    llm = get_llm()
...
- explanations must be concise but complete"""
    ...
    return {
        "questions": [
            {
                "id": 1,
                "question": f"Sample question on {topics_str} (AI generation failed — check model)",
                "options": ["Option A", "Option B", "Option C", "Option D"],
                "correct_answer": "A",
                "explanation": "Please check the AI service and model configuration.",
            }
        ]
    }
